...
//         const outputDir = path.join(__dirname, '../../../video', res);
//         await fs.promises.mkdir(outputDir, { recursive: true });
//         const outputPath = path.join(outputDir, `${slug}.mp4`);

//         console.log('Input file path:', filePath);
//         console.log('Video directory:', path.join(__dirname, '../../../video'));
//         console.log('Available resolutions:', Object.keys(resolutionConfig));
...
//             });
//             // Don't reject here, continue processing other resolutions
//             processedVideos++;
//           } else {
//             processedVideos++;
...
export const addVideoProcessingJob = (
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    ffmpeg.ffprobe(filePath, async (err, metadata) => {
      ...
      for (let i = selectedResolutionIndex; i >= 0; i--) {
        // const [res, config] = sortedResolutions[i];
        // const outputDir = path.join('/usr/src/app/video', res);
        // await fs.promises.mkdir(outputDir, { recursive: true });
        // const outputPath = path.join(outputDir, `${slug}.mp4`);
